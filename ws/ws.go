...
func (c *Client) Read() {
	...
	for {
		...
		go GetInstance().NewTask(c, GetInstance().Process(Message{
			Client:  c,
			Message: message,
		}))
	}
...
// 处理单个 client 发送数据
func (manager *Manager) SendService(ctx context.Context) {
	manager.Wg.Add(1)
L:
	for {
		select {
...
// 处理 group 广播数据
func (manager *Manager) SendGroupService(ctx context.Context) {
	manager.Wg.Add(1)
L:
	for {
		select {
		...
		case <-ctx.Done():
			log.Println("SendGroupService exit")
			break L
		}
	}
	manager.Wg.Done()
...
// 处理广播数据
func (manager *Manager) SendAllService(ctx context.Context) {
	manager.Wg.Add(1)
L:
	for {
		select {
		...
		case <-ctx.Done():
			log.Println("SendAllService exit")
			break L
		}
	}
	manager.Wg.Done()
...
func (manager *Manager) GetClientCount() uint {
	return manager.clientCount
}
